...
const DashboardAccount = ({intl}) => {
    ...
    {
        setStatus("Fail")
        setErrors({message: 'failed to fetch health information', color: '#DF4B31', icon: true})
        // localStorage.setItem("status","Pending");
    ...
    if (status === "Pending") {
        ...
        setErrors({message: 'Fetching health information...', color: 'primary', icon: true})
...
.catch(err => console.log(err))
    ...
    function getState(){ //alert("in func"+status)
        return status
    }
...
<Typography component={"h1"} variant={"h1"} className={classes.title}>
    ...
    {status==="Pending" ? <div><ErrorMessage errors={error} handleClose={handleCloseError}/><CircularProgress size={150} /></div> : null}
    {status==="Fail" ? <div><ErrorMessage errors={error} handleClose={handleCloseError}/></div> : null}

<HealthStats profile={profile}/>


        <HealthInfo profile={profile} dispatch={dispatch} newstatus={status}/>
